...
const ShivekKhuranaAndNewline = () => (
  <div className="flex justify-center items-center f7 f6-ns">
    <Link to="/authors/shivekkhurana" className="dib mh2 underline b">
      Shivek Khurana
    </Link>{" "}
    and
    <a href="https://newline.co" className="dib">
      <img
        className="h1 h2-ns mh2 white pt1"
        src={newlineLogo}
        alt="Newline.co Logo"
      />
    </a>
...
const RemarkableTool = () => (
  <div className="w-100 w-100-m w-40-l">
    <h3 className="georgia f3 f3-m f2-l normal mv0">
...
const WistiaEmbed = () => (
  <>
    <Head>
      <script
        src="https://fast.wistia.com/embed/medias/ler01csxi6.jsonp"
        async
      ></script>
      <script
    ...
    >
      ...
      >
        <div
          className="wistia_embed wistia_async_ler01csxi6 videoFoam=true"
          ...
          >
            <div
              dangerouslySetInnerHTML={{
                __html: `<img src="https://fast.wistia.com/embed/medias/ler01csxi6/swatch" style="filter:blur(5px);height:100%;object-fit:contain;width:100%;" alt="" aria-hidden="true" onload="this.parentNode.style.opacity=1;" />`,
              }}
            ></div>
          </div>
        </div>
...
const QAndA = ({ emoji, question, answer: Answer }) => (
  <div className="mt4 QAndA">
    ...
    <div className="mt3 lh-copy f6 f5-ns">
      <Answer />
    </div>
  </div>
);
...
const QAndAGrid = () => (
  <section id="faq" className="mt4 flex justify-between flex-wrap">
    {qAndAs.map((qa) => (
      <QAndA key={qa.question} {...qa} />
    ))}
  </section>
);
...
const partedPaidFeatures = array.partition(
  paidFeatures,
  paidFeatures.length / 2,
);
...
const Plans = () => (
  <div className="flex flex-column flex-row-ns justify-between">
    <div className="w-100 w-30-m w-30-l">
      <PricingPlanHeader price="$ 0.00" title="Reduce Mode" />
      {freeFeatures.map((f) => (
...
const Pricing = () => (
  <section id="pricing" className="ba bw2 br4 ph4 b--white-80 mt5 pb5">
...
const FreeResources = () => (
  <section id="more-resources" className="mt5 pb5">
    ...
    <p className="">
      ...
      through an extensive course. Here is our reccomendation on other free
      resources to jump-start your Clojure journey:
    </p>
    <div className="flex flex-wrap">
      {freeResources.map((r) => (
        <FreeResource key={r.url} resource={r} />
      ))}
    </div>
  </section>
);
...
const Tinycanva = () => (
  <>
    ...
    />
    <div className="white" style={{ backgroundColor: "#1B1B1B" }}>
